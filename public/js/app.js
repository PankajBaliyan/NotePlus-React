(function (jQuery) {
    ...
    $(document).on("change", '[data-change="radio"]', function (e) {
        ...
        if ($(this).attr("data-custom-target") == "color") {
            ...
            $("#note-icon").addClass(
                `icon iq-icon-box-2 icon-border-${value} rounded`
            );
            $("#update-note").addClass(
                `card card-block card-stretch card-height card-bottom-border-${value} note-detail`
            );
        }
    });
    ...
    $(document).on("click", ".edit-note", function () {
        $("#notebook-title").val(
            $(this).closest("tr").find("[data-column]").text()
        );
    });
})(jQuery);
